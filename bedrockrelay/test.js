...
async function run() {
  ...
  const reboundHost = dgram.createSocket('udp4');
  const attacker = dgram.createSocket('udp4');
  const client = dgram.createSocket('udp4');
  const client2 = dgram.createSocket('udp4');
  let token;
  try {
    await Promise.all([bind(host), bind(reboundHost), bind(client), bind(client2), new Promise(resolve => attacker.bind(0, '127.0.0.2', resolve))]);
    ...
    reboundHost.send('BRLY_HOST', session.port, '127.0.0.1');
    if ((await receive(reboundHost)).data.toString() !== 'BRLY_OK') throw new Error('Host UDP portu yenilenemedi.');

    client.send('PING1', session.port, '127.0.0.1');
    const first = (await receive(reboundHost)).data;
    client2.send('PING2', session.port, '127.0.0.1');
    const second = (await receive(reboundHost)).data;
    ...
    reboundHost.send(Buffer.concat([first.subarray(0, 2), Buffer.from('PONG1')]), session.port, '127.0.0.1');
    reboundHost.send(Buffer.concat([second.subarray(0, 2), Buffer.from('PONG2')]), session.port, '127.0.0.1');
...
run().catch(error => { console.error(`Hata ${error.message}`); process.exitCode = 1; });
