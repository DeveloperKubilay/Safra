...
public final class SafraDirectConnectScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.ipField = new GuiTextField(this.fontRendererObj, this.width / 2 - 100, 116, 200, 20);
        this.ipField.setMaxStringLength(200);
        this.ipField.setText(shownAddress);
        this.ipField.setFocused(true);

        this.buttonList.add(this.selectButton = new GuiButton(0, this.width / 2 - 100, this.height / 4 + 96, 98, 20, I18n.format("selectServer.select")));
        this.buttonList.add(new GuiButton(1, this.width / 2 + 2, this.height / 4 + 96, 98, 20, I18n.format("gui.cancel")));
        this.buttonList.add(new GuiButton(2, this.width / 2 - 100, this.height / 4 + 120, 200, 20, getToggleText()));
        updateValidation();
    }
    ...
    private void connect() {
        ...
        try {
            ...
            this.mc.displayGuiScreen(new net.minecraft.client.gui.GuiDisconnected(parent, "connect.failed", new ChatComponentTranslation("safra.p2p.prepare_failed", message)));
        } catch (CompletionException exception) {
            Throwable cause = exception.getCause() == null ? exception : exception.getCause();
            if (cause instanceof CancellationException) {
                return;
            }
            this.mc.displayGuiScreen(new net.minecraft.client.gui.GuiDisconnected(parent, "connect.failed", new ChatComponentText(cause.getMessage())));
        }
    }
    ...
    private void updateValidation() {
        if (this.selectButton != null) {
            ...
            this.selectButton.enabled = this.p2pEnabled ? P2pManager.isValidP2pAddress(address) : ServerAddress.func_78860_a(address) != null;
        }
    }
