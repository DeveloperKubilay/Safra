...
public final class SafraGuiShareToLan extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList.add(this.gameModeButton = new GuiButton(0, this.width / 2 - 155, this.height - 112, 150, 20, getGameModeText()));
        this.buttonList.add(this.commandsButton = new GuiButton(1, this.width / 2 + 5, this.height - 112, 150, 20, getAllowCommandsText()));
        this.buttonList.add(this.p2pButton = new GuiButton(2, this.width / 2 - 155, this.height - 88, 150, 20, getP2pText()));
        this.buttonList.add(this.onlineModeButton = new GuiButton(3, this.width / 2 + 5, this.height - 88, 150, 20, getOnlineModeText()));
    ...
    private void openLan() {
        ...
        P2pManager.getInstance().startHostingAsync(tcpPort, fixedCode, () -> mc.func_152344_a(new Runnable() {
            @Override
            public void run() {
        ...
        })).whenComplete((shareCode, throwable) -> mc.func_152344_a(new Runnable() {
            @Override
            public void run() {
                if (throwable != null) {
    ...
    private WorldSettings.GameType resolveGameType() {
        WorldSettings.GameType type = WorldSettings.getGameTypeById(0);
        if ("creative".equals(gameMode)) {
            type = WorldSettings.GameType.CREATIVE;
        } else if ("adventure".equals(gameMode)) {
            type = WorldSettings.GameType.ADVENTURE;
        }
