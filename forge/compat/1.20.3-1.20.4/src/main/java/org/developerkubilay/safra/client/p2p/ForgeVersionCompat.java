...
public final class ForgeVersionCompat {
    ...
    public static Minecraft getMinecraftInstance() {
        Object resolved = invokeStaticAccessor(Minecraft.class, Minecraft.class, "m_91087_", "getInstance");
        if (resolved instanceof Minecraft minecraft) {
            return minecraft;
        }

        throw new IllegalStateException("Could not find a compatible Minecraft instance accessor");
    }
    ...
    private static void copyServerSettings(ServerData target, ServerData source) {
    ...
    private static Object resolveThirdServerDataArgument(ServerData originalServerInfo, Class<?> thirdParameterType) {
        if (thirdParameterType == boolean.class || thirdParameterType == Boolean.class) {
            ...
            Object lanField = getFieldValue(originalServerInfo, "lan", "f_105364_");
            if (lanField instanceof Boolean booleanValue) {
                return booleanValue;
            }
            return false;
        }
    ...
    private static Object invokeStaticAccessor(Class<?> target, Class<?> expectedType, String... names) {
        Class<?> type = target;
        while (type != null) {
            for (String name : names) {
                try {
                    ...
                    if (!Modifier.isStatic(method.getModifiers())
                        ...
                        || !expectedType.isAssignableFrom(method.getReturnType())) {
                        continue;
                    }
                    method.setAccessible(true);
