...
public final class P2pHostService implements AutoCloseable {
    ...
    private synchronized void ensureRelayAvailable(InetSocketAddress joinerRelayAddress) {
        ...
        try {
            ...
            if (publishRelayReady(relayBinding.publicEndpoints())) {
                notifyRelayReady();
            }
    ...
    private boolean publishRelayReady() {
        if (!(relayFallbackTransport instanceof org.developerkubilay.safra.p2p.turn.P2pTurnDatagramTransport turnTransport)) {
            return false;
        }
        return publishRelayReady(java.util.List.of(turnTransport.relayAddress()));
    }

    private boolean publishRelayReady(Collection<InetSocketAddress> publicEndpoints) {
