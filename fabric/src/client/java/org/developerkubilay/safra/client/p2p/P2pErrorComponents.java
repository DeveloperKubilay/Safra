...
public final class P2pErrorComponents {
    ...
    public static Text disconnectFailure(Text details) {
        ...
        if (context.kind() != P2pErrorKind.OTHER) {
            return safraError(new TranslatableText(context.kind().translationKey()));
        }
    ...
    private static Text safraErrorWithDiscord(Text details) {
        ...
        return new LiteralText("Safra Error: ")
            .append(details)
            .append(new LiteralText("\n" + discordUrl).setStyle(new Style()
                .setColor(Formatting.BLUE)
                .setUnderline(true)
                .setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, discordUrl))
            ));
    }
}
