<p align="center">
  <img src="https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/parca1.png" alt="parca1" width="24%" />
  <img src="https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/parca2.png" alt="parca2" width="24%" />
  <img src="https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/parca3.png" alt="parca3" width="24%" />
  <img src="https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/parca4.png" alt="parca4" width="24%" />
</p>

<p align="center">
  <img src="https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/readme.gif" alt="alt text" />
</p>

![alt text](https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/image1.png)

# Safra 🚀

No pierdas tiempo con la configuracion del servidor ni con pasos innecesarios.  
Inicia tu servidor de Minecraft con **2 clics** y entra directo al juego.

✅ Sin interfaz complicada 😌  
✅ Sin sistemas innecesarios 🚫  
✅ Ligero y optimizado ⚡  

![alt text](https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/screenshot2.png)

> ✅ En la vista previa se usa **TLauncher** (tambien se admiten launchers alternativos)  
> ✅ Incluye soporte para **chat de voz** (soporte de mod)

## Compatibilidad con mods

Puedes jugar facilmente con los mods que tu y tus amigos tengan instalados.  
Compatible con **Fabric**, **Forge** y **NeoForge**.  
Se probo con mas de 180 mods y funciono sin problemas.

## Nota ⚠️

> Si van a entrar jugadores con TLauncher u otro launcher, basta con que la persona que inicia el servidor ponga `online-mode=false` antes de compartir la partida.

![alt text](https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/screenshot3.png)

> Si quieres que el codigo generado permanezca fijo, solo activa `Use Fixed Code=on` en Configuracion.

![alt text](https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/screenshot8.png)

## Soporte para Bedrock

![alt text](https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/bedrock.png)

> Necesitaras el mod Geyser. Sin embargo, este no es el enfoque principal del mod, por lo que los servidores pueden estar ocupados.

## FAQ 🤔

<table>
  <tr>
    <td width="30%" align="center">
      <img src="https://raw.githubusercontent.com/DeveloperKubilay/Safra/refs/heads/assets/png/emoji1.png" alt="faq emoji" width="100%" />
    </td>
    <td width="70%" align="left">
      ✨ Necesitas una funcion, tienes una idea para otra version o encontraste un bug?<br />
      Puedes contactarnos en cualquier momento a través de <a href="https://discord.gg/NHjBvRxDXP">Discord</a>.<br /><br />
      ❤️ El proyecto con código accesible vive aqui<br />
      <a href="https://github.com/DeveloperKubilay/Safra">DeveloperKubilay/Safra</a><br /><br />
      <a href="https://github.com/DeveloperKubilay/Safra/blob/assets/languages/es/how_it_works.md">👀 Quieres ver como funciona?</a>
    </td>
  </tr>
</table>
